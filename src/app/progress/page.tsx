...
export default async function ProgressPage() {
  ...
  try {
    ...
    if (dbUser?.level) userLevel = dbUser.level;
  } catch (err) {
    console.error("Failed to query user role in progress page:", err);
  }

  const profile = await getPersonalizedLearningProfile(sessionUser.email);
  const userName = sessionUser.name || profile?.userName || "Sarah Jenkins";
  const userInitials =
    userName
      .split(" ")
      .map((n: string) => n[0])
        ...
        <div className="p-6 border-t border-indigo-900/60 bg-indigo-950/50">
          <div className="flex items-center gap-3">
            ...
            <div className="text-xs text-indigo-200">
              ...
              <p className="opacity-75">{profile?.completionPercentage || 0}% curriculum completed</p>
            </div>
          </div>
        </div>
      </aside>
